      ...
      {willData && (
        <>
                  ...
                  <div className="space-y-3">
                    {willData.heirs.map((heir, index) => (
                      <div key={index} className="border-b pb-3 last:border-0">
                        ...
                        {heir.share && <p className="text-sm text-gray-600">Доля: {heir.share}</p>}
                      </div>
                    ))}
                  </div>
                </CardContent>
          ...
          <div className="text-sm text-gray-500">
            <p>Создано: {new Date(willData.createdAt).toLocaleString('ru-RU')}</p>
            {willData.updatedAt !== willData.createdAt && (
              <p>Обновлено: {new Date(willData.updatedAt).toLocaleString('ru-RU')}</p>
            )}
          </div>
        </>
      )}
