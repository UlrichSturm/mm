...
export default function SelectLawyerPage() {
  ...
  const filteredAndSortedLawyers = lawyers
    .filter(lawyer => {
      ...
      if (licenseFilter !== 'all') {
        if (
          licenseFilter === 'lawyer' &&
          lawyer.licenseType !== 'lawyer' &&
          lawyer.licenseType !== 'both'
        ) {
          return false;
        }
        if (
          licenseFilter === 'notary' &&
          lawyer.licenseType !== 'notary' &&
          lawyer.licenseType !== 'both'
        ) {
          return false;
        }
  ...
  if (error) {
    return (
      <div className="container mx-auto px-4 py-8">
        <ErrorDisplay error={error} onRetry={loadLawyers} title={t('wills.lawyer.loadError')} />
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <p className="text-muted-foreground text-center mb-8">{t('wills.lawyer.subtitle')}</p>
        ...
        <CardContent className="p-4 sm:p-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="w-full md:w-48">
              <Select
                value={sortBy}
                onValueChange={value => setSortBy(value as 'rating' | 'distance')}
              >
                <SelectTrigger className="h-12">
                  <SelectValue />
                </SelectTrigger>
