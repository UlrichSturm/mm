...
**Как Vendor**, я хочу зарегистрироваться на платформе через Keycloak, чтобы размещать свои услуги

---

## Acceptance Criteria

- [ ] Endpoint `POST /auth/register` принимает данные регистрации (vendor регистрируется через тот же endpoint)
- [ ] Дополнительные поля: businessName, phone, address, postalCode (для vendor)
- [ ] Валидация всех обязательных полей
- [ ] Создание пользователя в Keycloak через Admin API
- [ ] Назначение роли `vendor` в Keycloak
- [ ] Создание User в локальной БД с ролью VENDOR
...
6. ✅ Пользователь создан в Keycloak с ролью `vendor`
7. ✅ Пользователь синхронизирован с локальной БД
8. ✅ VendorProfile создан со статусом PENDING
